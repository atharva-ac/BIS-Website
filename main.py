...
class QueryRequest(BaseModel):
    question: str = Field(..., description="The query string to search in the BIS document", example="What standard applies to plugs and sockets?")
...
def query_bis_document(request: QueryRequest):
    ...
    try:
        result = query_rag(query_text=request.question, top_k=request.top_k)
        return result
...
def trigger_reingestion(background_tasks: BackgroundTasks):
    ...
    try:
        background_tasks.add_task(build_and_save_vector_store)
