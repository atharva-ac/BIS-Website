...
(function (global) {
  ...
  function wireChrome() {
    ...
    // Support button - matched on either the data-support attribute some pages
    // set, or the shared data-nav="Support" label every page's sidebar uses
    // (a couple of pages only had the latter, which left Support inert there).
    ...
    // static Standards catalog. Pages with their own purpose-built search
    // (Standards Discovery's catalog filter, Lab Finder's local filter) use
    // a different attribute (data-page-search) and are deliberately excluded.
    ...
    // New Analysis via any element with data-new-analysis-action
  ...
  /* ------------------------- RAG API (BIS Assistant) --------------------- */
  /* Talks to the FastAPI + Chroma + Ollama strict-RAG backend (BIS-Website-main/main.py,
     default http://127.0.0.1:8080). Only assistant.html uses this — every other
     page/feature is untouched. On failure the callback receives an Error; callers
     must surface that, never fabricate an answer. */
  ...
  // Kept for any external reader that expects a static list (equals the
  // no-override default). Live calls always use getRagBaseUrls().
  ...
  function ragQuery(question, topK, history, callback) {
    ...
    if (history && history.length) {
      // Backend only uses the last few turns and only to resolve references
      // like "it" - trim client-side too so we're not shipping the whole thread.
    ...
    function tryNext() {
      if (tried >= baseUrls.length) {
        callback(new Error('Could not reach the BIS RAG API on any known port (8080/8000). Run `python main.py` inside BIS-Website-main, or check the backend URL in Settings.'));
        return;
      }
      ...
      // Local LLM generation typically takes 15-30s on CPU, more for longer
      // answers. 45s left near-zero margin and caused false "connection
      // error" reports on normal (if slow) responses - measured.
      xhr.timeout = 90000;
      xhr.onload = function () {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          // Client-side validation error (e.g. question too long) - this will
          // fail identically on every base URL, so surface it immediately
          // instead of wasting time retrying the other ports.
          try {
